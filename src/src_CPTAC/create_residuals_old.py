...
def load_and_preprocess_data():
    ...
    """
    # Load data
    cptac_pancan_prot = pd.read_parquet("./CPTAC_PanCan_proteomics.parquet")
    cptac_pancan_prot.index.name = 'sample_ID'

    cptac_pancan_trans = pd.read_parquet("./CPTAC_PanCan_transcriptomics.parquet")
    cptac_pancan_trans.columns.name = ''
    cptac_pancan_trans.index.name = 'sample_ID'

    cptac_cnv_wide = pd.read_csv('./cptac_gistic_bcm_20nov23.csv')
...
def merge_dataframes(prot_long, trans_long, cnv_long):
    ...
    cptac_sample_info = pd.read_csv('./cptac_tcga_sample_info_updated.csv').rename(columns={'Unnamed: 0': 'sample_ID'})
...
def regress_out_variables_optimized(df, dependent_var, independent_vars):
    """
    ...
    for gene in tqdm(df['Gene'].unique(), desc="Processing Genes"):
        gene_df = df.loc[df['Gene'] == gene, ['Sample', dependent_var] + independent_vars].dropna()
        if len(gene_df) > 1:
            formula = f'{dependent_var} ~ ' + ' + '.join(independent_vars)
            model = smf.ols(formula, data=gene_df).fit()
            gene_df['Residual'] = model.resid
            for _, row in gene_df.iterrows():
                residuals_key = f"{gene}_in_{row['Sample']}"
                residuals_dict[residuals_key] = row['Residual']
